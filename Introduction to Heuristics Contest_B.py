...
for i in range (1,D):
    ...
    _noheldsdaylist=noheldsdaylist

    _noheldsday_today=list(map(lambda x: x+1,_noheldsdaylist[i-1]))
    _noheldsday_today[tlist[i]]=0
    unpleasant=list(map(mul,clist,_noheldsday_today))

    v=slist[i][tlist[i]]-sum(unpleasant)

    _noheldsday.append(_noheldsday_today)
    _benefitlist.append(v)
    benefitlist=_benefitlist
    noheldsdaylist=_noheldsday
