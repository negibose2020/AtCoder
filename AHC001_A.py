...
def CutExcessPart_horizon(x,y,x1,y1,x2,y2,end_x,end_y,r,n):
    w=x2+1-x1
    h=y2+1-y1
    area=w*h
    x1,y1,x2,y2=map(int,[x1,y1,x2,y2])
    movetoleft=(area-r)//h
    if x+1<=end_x-movetoleft:
        x2=end_x-movetoleft
    else:
        x2=x+1
    return x2

def CutExcessPart_vertic(x,y,x1,y1,x2,y2,end_x,end_y,r,n):
    w=x2+1-x1
    h=y2+1-y1
    area=w*h
    x1,y1,x2,y2=map(int,[x1,y1,x2,y2])
    movetoup=(area-r)//w
    if y+1<=end_y-movetoup:
        y2=end_y-movetoup
    else:
        y2=y+1
    return y2
